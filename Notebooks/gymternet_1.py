...
years = [2023, 2022, 2021, 2020, 2019, 2018, 2017, 2016, 2015] # These are the years that we are interested in evaluating
...
for row in rows:
    ...
    a_href = row.css('a::attr(href)').get()
    a_text = row.css('a::text').get()
    division = row.css('div > div:nth-child(8)::text').get()
    
    #print(a_selector)
    print(a_text)
    print(a_href)
    team_names.append(a_text)
    team_links.append(a_href)
